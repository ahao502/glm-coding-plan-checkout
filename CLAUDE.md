...
This is a Node.js ESM CLI (`"type": "module"`) that uses Playwright to automate creating an unpaid `Pro` monthly-recurring checkout link on `bigmodel.cn/glm-coding`. It opens a browser shortly before 10:00 UTC+8 and retries the checkout action during a 10:00-10:15 window.
...
- **`src/scheduler.js`** — Calculates the next 10:00-10:15 window. Passes `startAt`/`stopAt` to `browser-flow.js`'s `runFastClickCheckout`. Resolves `GLM_RETRY_INTERVAL_MS` from env.
